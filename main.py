...
def criticUpdate(critic_nets, critic_optim, target_actor_nets, target_critic_net, states, all_next_obs_agents, acts, rewards, dones, kwargs):
    critic_optim.zero_grad()
    joint_states = torch.cat(states, dim=-1)
    joint_act = torch.cat(acts, dim=-1)
    joint_next_obs = torch.cat(all_next_obs_agents, dim=-1)

    predicted_q_val_1 = getQValues(critic_nets[0], joint_states, joint_act)
    predicted_q_val_2 = getQValues(critic_nets[1], joint_states, joint_act)
    noise = GaussianNoise(8, sigma=0.2)

    next_acts = step_with_exp(all_next_obs_agents, target_actor_nets, noise=noise)
    next_input = torch.cat((joint_next_obs, torch.cat(next_acts, dim=-1)), dim=-1)
...
def update(actor_nets, critic_nets1, critic_nets2, target_actor_nets, target_critic_nets1, target_critic_nets2,
           exp_replay, actor_optimizers, critic_optimizers, kwargs):
...
def main(**kwargs):
    ...
    actor_nets = [MLP(obs_dim[i], [400, 400], action_dim[i]) for i in range(2)]
    critic_nets1 = [MLP((obs_dim[0]+obs_dim[1]+action_dim[0]+action_dim[1]), [400, 400], 1, net_type="Critic") for
                   i in range(2)]
    critic_nets2 = [MLP((obs_dim[0]+obs_dim[1]+action_dim[0]+action_dim[1]), [400, 400], 1, net_type="Critic") for
                   i in range(2)]
    target_actor_nets = [MLP(obs_dim[i], [400, 400], action_dim[i]) for i in range(2)]
    target_critic_nets1 = [MLP((obs_dim[0]+obs_dim[1]+action_dim[0]+action_dim[1]),
                               [400, 400], 1, net_type="Critic") for i in range(2)]
    target_critic_nets2 = [MLP((obs_dim[0]+obs_dim[1]+action_dim[0]+action_dim[1]),
                               [400, 400], 1, net_type="Critic") for i in range(2)]
    ...
    for epsNum in range(kwargs['num_episodes']):
        ...
        while dones[0][0] is False and dones[0][1] is False:
            ...
            exp_replay.push(np.asarray(observation), action_numpy, np.asarray(rewards), np.asarray(next_obs_given),
                            np.asarray(dones))
            if totalSteps % kwargs['train_frequency'] == 0:
                update(actor_nets, critic_nets1, critic_nets2, target_actor_nets, target_critic_nets1,
                       target_critic_nets2, exp_replay, actor_optimizers, critic_optimizers, kwargs)
                for actor_network, target_network in zip(actor_nets, target_actor_nets):
